...
public:
    ...
    void set_edge_id(uint32_t edge_id) {
        _edge_id = edge_id;
    }

    uint32_t get_edge_id() const {
        return _edge_id;
    }

    void set_source(uint32_t start_node) {
        _source_node = start_node;
    }
    ...
    void set_target(uint32_t target_node) {
    ...
    HistCost& get_cost() const {
        return *cost;
    }

    HistCost* get_cost_pointer() const {
        return cost;
    }
