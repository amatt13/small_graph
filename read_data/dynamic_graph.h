...
private:
    ...
    // ForwardEdge has information about cost.
    struct ForwardEdge
    {
        ...
        void set_up(EdgeCost&& edge_info)
        {
    ...
    void remove_in_edge(const NodeIterator& tgt, const BwEdgeIterator& e)
    {
        ...
        // Reduce count on amount of edges to this node.
        ...
        // If the last index wasnt the edge to be removed, move the edges around.
        if ( e != last_edge_it )
        {
...
public:
    ...
    // Used by the run_ordering.cpp on line 77 and ordering 67
    template <typename EdgeCost>
    DynamicGraph(std::vector<EdgeCost>&& edge_list)
    : _nodes(), _forward_edges(), _backward_edges(), _n_edges(edge_list.size())
    {
        size_t max_node_it = 0;

        // max_node_it is used to figure out the maximum amount of nodes the graph can contain.
        // This logic is used as the input file contains edge information for source/target with node id.
        std::for_each( edge_list.begin(), edge_list.end(),
                        [&max_node_it](const EdgeCost& edge_info) -> void
                        {
                            max_node_it = std::max(max_node_it, edge_info.get_source());
                            max_node_it = std::max(max_node_it, edge_info.get_target());
                        } );
        //The _nodes array is allocated space for the (maximum) amount of nodes. Each item takes up Node() amount of space.
        _nodes.assign(max_node_it + 1, Node());

        //
        // set up forward edges for each node in _nodes
        // No need to change this.

        {
            // Sort edges in edge_list such that lowest source is first.
            // After this sorting we can simply get start and end index for edges going from specific sources.
            std::sort( edge_list.begin(), edge_list.end(),
                            [](const EdgeCost& lhs, const EdgeCost& rhs) -> bool
                                    { return lhs.get_source() < rhs.get_source(); } );
            // Outer loop going through edges one at a time from start to end. "it" is an iterator for the vector.
            for ( auto it = edge_list.begin() ; it != edge_list.end() ; ++it )
            {
                // Check to see if we have already looked at this node's out_edges.
                if ( _nodes[it->get_source()]._out_edges_begin == INVALID_FW_EDGE_ITERATOR )
                {
                    // insert dummy edges if we go to the else statements(push_back)
                    if ( it > edge_list.begin() )
                    {
                        NodeIterator previous_source = (it-1)->get_source();
                        //Below is an expression to get the amount of out-edges the previous node had based on start and end indices.
                        ...
                        // make dummy edges based on amount of out-edges on previous node
                ...
                // Creates a forward edge and sets up the data according to the i'th element in edge_list
                //auto fwedge = ForwardEdge();
                //fwedge.set_up(std::move(*it));
                _forward_edges.push_back(std::move(ForwardEdge()));
                _forward_edges.back().set_up( *it );
        ...
        //
        // set up backward edges for each node in _nodes
        // No need to change this.

        {
            struct BackwardEdgeCost
            {
                NodeIterator _source;
                NodeIterator _target;
                FwEdgeIterator _forward_edge;
            ...
            std::vector<BackwardEdgeCost> backward_edge_list;
            ...
            for ( NodeIterator source = 0 ; source != _nodes.size() ; ++source )
            {
                for ( FwEdgeIterator fw_edge_it = _nodes[source]._out_edges_begin ; fw_edge_it < _nodes[source]._out_edges_end ; ++fw_edge_it )
                {
                    ...
                    backward_edge_list.push_back(BackwardEdgeCost());
            ...
            std::sort(
                backward_edge_list.begin(), backward_edge_list.end(),
                    [](const BackwardEdgeCost& a, const BackwardEdgeCost& b) -> bool { return a._target < b._target; }
            );
    ...
    {
        // The vector edges is appended with all backward edges from _nodes
        ...
        return
            util::has_duplicates( // This method can be found @line 76 in misc.h
                edges.begin(),
                edges.end(),
                ...
                [&] (const BackwardEdge& lhs, const BackwardEdge& rhs) -> bool // "equal_to" operator
                {
                    ...
                    if ( _forward_edges[lhs._forward_edge]._target != _forward_edges[rhs._forward_edge]._target )
                        return false;
                    KATCH_STATUS("Found duplicates: " << lhs._source <<  " " <<_forward_edges[lhs._forward_edge]._target << " "<< rhs._source << " " << _forward_edges[rhs._forward_edge]._target << "\n");
                    return true;
                }
       );
    }
    ...
    const OrderEdgeData& get_edge_data(FwEdgeIterator e) const
    {
        assert( e < _forward_edges.size() );
        assert( ! _forward_edges[e].is_dummy() );
    ...
    OrderEdgeData& get_edge_data(FwEdgeIterator e)
    {
        return const_cast<OrderEdgeData&>(static_cast<const DynamicGraph&>(*this).get_edge_data(e));
    }

    const OrderEdgeData& get_edge_data(BwEdgeIterator e) const
    {
        assert( e < _backward_edges.size() );
        assert( ! _backward_edges[e].is_dummy() );
    ...
    OrderEdgeData& get_edge_data(BwEdgeIterator e)
    {
        return const_cast<OrderEdgeData&>(static_cast<const DynamicGraph&>(*this).get_edge_data(e));
    }
    // Traverse through edges, searching for an edge from src to tgt.
    ...
    std::pair<FwEdgeIterator,BwEdgeIterator> insert_edge(const NodeIterator& src, const NodeIterator& tgt, OrderEdgeData&& data)
    {
    ...
    void remove_all_edges_of_node(const NodeIterator& u)
    {
        ...
        for ( FwEdgeIterator e_fw = _nodes[u]._out_edges_begin ; e_fw != _nodes[u]._out_edges_end ; ++e_fw )
        {
            ...
            if(_forward_edges[e_fw]._target == u){
                KATCH_STATUS("\n" << _forward_edges[e_fw]._target << "\n " );
            }
